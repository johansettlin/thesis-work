...
def addNewAssociation(g, obj1, obj2, linkName, r1):
    ...
    for asset in assocs:
        ...
        if not ((target == inheritedFrom1 or start == inheritedFrom1) and (target == inheritedFrom2 or start == inheritedFrom2)):
            print("The association does not exits")
            valid = False
    print("is it valid in addnewassoc? ", valid)
    if(valid):
        roleAndCardObj1 = g.V().hasLabel(inheritedFrom1).out("associations").hasLabel(linkName).has('role', r1).project("role", "card").by("role").by("cardinality_begin").next()
        roleAndCardObj2 = g.V().hasLabel(inheritedFrom2).out("associations").hasLabel(linkName).not_(__.has('role', r1)).project("role", "card").by("role").by("cardinality_begin").next()
        # else:
...
def getCardinalityForAssoc(g, meta1, meta2, role):
    if(not g.V().hasLabel(meta2).out("associations").has('role', role).hasNext()):
        return []
    if(not g.V().hasLabel(meta2).out("associations").has('role', role).where(__.out("targetType").hasLabel(meta1)).hasNext()):
        return []
    card = g.V().\
            hasLabel(meta2).\
            out("associations").\
...
#Remake to support extends
    ...
    for e1 in o1E:
        ...
        for e2 in o2E:
            if(not hasLink):
                if(g.V().hasLabel(e2).out("associations").where(__.and_(__.has("role", role2),__.out("targetType").hasLabel(e1))).hasNext()):
